...
public class TimerCalculator {
    ...
    @Pointcut("execution(* com.bingdou.oss.controller.*.*(..))")
        ...
        try {
            ...
            if (StringUtils.isEmpty(requestMethod)) {
                ...
                if (requestMethod.contains("_"))
                    method = methodName + "/ossserver-" + requestMethod.replaceAll("_", "-");
                else
                    method = methodName + "/ossserver-" + requestMethod;
            }
        } catch (Throwable e) {
            throw e;
        } finally {
