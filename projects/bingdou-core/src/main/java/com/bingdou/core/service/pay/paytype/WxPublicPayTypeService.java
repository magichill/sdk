...
/**
 * Created by gaoshan on 17/3/6.
 */
@Service
public class WxPublicPayTypeService extends BasePayTypeService implements IPayTypeService {
    ...
    protected String getRequestOrderResult(Object param) throws Exception {
        return HttpClientUtil.doPostJsonOrXmlHttpClient("call-wx-server-pay", PayTypeData.PAY_TYPE_WX_URL,
    ...
    protected PayTypeResponse getRequestOrderResponse(String payTypeResult) {
        ...
        if (map == null || map.isEmpty()) {
            LogContext.instance().error("微信公众号订单预处理结果为空");
            return null;
        }
        ...
        String appId = PayTypeData.PAY_TYPE_WX_APP_ID_H5;
        String partnerId = PayTypeData.PAY_TYPE_WX_MCH_ID_H5;
    ...
    protected boolean isValidCallBackResponse(Map<String, String> map) {
        ...
        if (!mySign.equals(sign)) {
            LogContext.instance().error("公众号回调签名验证失败");
            return false;
        }
    ...
    private String getSign(Map<String, String> postParamsMap) {
        ...
        return DigestUtils.md5Hex(signStr + "&key=" + PayTypeData.PAY_TYPE_WX_KEY_H5).toUpperCase();
    }

    private Map<String, String> buildParamMap(PayTypeRequest payTypeRequest, String nonceStr) {
        Map<String, String> map = new HashMap<String, String>();
        map.put("appid", PayTypeData.PAY_TYPE_WX_APP_ID_H5);
        map.put("mch_id", PayTypeData.PAY_TYPE_WX_MCH_ID_H5);
        map.put("openid", payTypeRequest.getOpenId());
        ...
        if (payTypeRequest.isRecharge()) {
            map.put("notify_url", PayTypeData.PAY_TYPE_WX_PUBLIC_RECHARGE_NOTIFY_URL);
        } else {
            map.put("notify_url", PayTypeData.PAY_TYPE_WX_PUBLIC_CONSUME_NOTIFY_URL);
        }
        map.put("trade_type", PayTypeData.PAY_TYPE_WX_TRADE_TYPE_H5);
